...
const experiments = [
      ...
      },
      {
            ...
            inputs: ["voltage", "conductivity", "impurityConc", "temperature"]
      },
      {
...
const GalvanicCalculator = () => {
      ...
      // ✅ NEW state to toggle graph visibility
      ...
      const calculateResults = () => {
            try {
                  ...
                  if (selectedExp.type === "galvanic") {
                        ...
                        for (let i = 1; i <= 50; i++) {
                              const conc = 0.001 + i * 0.1;
                              labels.push(conc.toFixed(2));
                              dataPoints.push(
                                    standardCellPotential -
                                    (0.0591 / selectedExp.electrons) *
                                    Math.log10(conc / inputs.anodeConc)
                              );
                        }
                  ...
                  // ✅ Show results but NOT the graph by default
                  setResults({
                  ...
                  // Reset the graph visibility so user must click toggle button again
                  setShowGraph(false);
            } catch (error) {
      ...
      return (
            <Box sx={{ padding: 4, direction: "rtl", marginTop: "20px" }}>
                  ...
                  {/* ✅ Button to Calculate (Shows Results) */}
                  <Button
                        variant="contained"
                        color="primary"
                        fullWidth
                  ...
                  {/* ✅ Button to Toggle Graph Visibility (Only if results exist) */}
                  {results && (
                        <Button
                              variant="outlined"
                              color="secondary"
                  ...
                  {/* ✅ Conditionally Show Graph If Toggled ON */}
                  {showGraph && graphData.labels.length > 0 && (
